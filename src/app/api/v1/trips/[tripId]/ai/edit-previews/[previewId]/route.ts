...
import {
  applyItineraryEditPreviewService,
  discardItineraryEditPreviewService,
} from "@/server/application/ai/itinerary-editing";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await applyItineraryEditPreviewService({
      userId,
      tripId,
      previewId,
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  try {
    const userId = await requireSessionUserId();
    const { tripId, previewId } = await context.params;
    const result = await discardItineraryEditPreviewService({
      userId,
      tripId,
      previewId,
    });
